...
public sealed class AdsConnectorTests
{
    ...
    private sealed class TestConnector(string platform) : IAdsPlatformConnector
    {
        ...
        public Task<AdsMetricSnapshot?> FetchMetricsAsync(string externalCampaignId, CancellationToken ct = default) => Task.FromResult<AdsMetricSnapshot?>(null);
        public Task<bool> ApplyActionAsync(string externalCampaignId, string action, decimal? newBudget, CancellationToken ct = default) => Task.FromResult(false);
        public Task<string?> BuildLookalikeAsync(IReadOnlyList<string> seedContactKeys, CancellationToken ct = default) => Task.FromResult<string?>(null);
        public Task<bool> BuildRemarketingAsync(string audienceName, IReadOnlyList<string> contactKeys, CancellationToken ct = default) => Task.FromResult(false);
    }
}
