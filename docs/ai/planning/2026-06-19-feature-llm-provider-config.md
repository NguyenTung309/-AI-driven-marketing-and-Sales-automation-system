...
### Review-driven fixes (from /code-review 2026-06-22)
- [x] **R1 — Permission-gate agent settings/binding writes** *(S)* — done 2026-06-22: mutating `/api/agents` routes (`enable`, `disable`, `settings` PUT, `sandbox`) now require existing seeded `agent.manage`; added `AgentsEndpointPermissionTests`. Targeted API tests passed: `AgentsEndpointPermissionTests|LlmConfigValidationTests` (32/32).
- [x] **R2 — Harden provider `baseUrl` SSRF guard beyond literal IPs** *(M)* — done 2026-06-22: extracted shared `LlmBaseUrlGuard`, rejects DNS hosts resolving to private/local addresses or DNS failures, fails closed, blocks userinfo/multicast/site-local cases, and revalidates/pins custom `BaseUrl` connections via cached guarded `HttpClient` for Anthropic + OpenAI-compatible clients. Provider/baseUrl changes now clear the stored key + deactivate config until key rotation, preventing old-key exfiltration to a new endpoint; activation/test paths reject missing/corrupt key safely. Added DNS/private-host, runtime factory, and domain key-rotation tests. Targeted tests passed: `LlmConfigValidationTests` (30/30), `LlmBaseUrlGuardTests|LlmChatClientFactoryTests` (6/6), `LlmConfigTests` (2/2).
- [x] **R3 — Revalidate bound agents when LLM config provider/model changes** *(M)* — done 2026-06-22: `UpdateAsync` now queries non-deleted agents bound to the config and rejects updates with `model_provider_mismatch` if the config model or any effective bound-agent model would be incompatible with the target provider before mutating the row. Added helper tests for compatible, incompatible, blank-agent-model fallback, and config-model mismatch cases. Targeted tests passed: `LlmConfigValidationTests` (32/32).
- [x] **R4 — Send Anthropic `temperature` from resolved config** *(S)* — done 2026-06-22: `AnthropicChatClient` now serializes nullable `temperature` from `ResolvedLlmConfig` into the messages payload (omitted when null). `AnthropicChatClientTests` asserts payload value. Targeted tests passed: `AnthropicChatClientTests` (2/2).
- [x] **R5 — Redact chat/sale-assist history before outbound LLM calls and sandbox trace persistence** *(M)* — done 2026-06-22: `ChatAgent` now redacts history before non-streaming and streaming LLM calls; `SaleAssistAgent` now redacts recent turns before draft, summary, upsell, and auto-summary/summarizer calls; agent sandbox traces now persist redacted input/reply. Added chat + sale-assist redaction tests and sandbox source guard. Targeted tests passed: `ChatAgentWiringTests|SaleAssistAgentRedactionTests` (14/14), `AgentsEndpointPermissionTests` (6/6). `SaleAssistAgentGrpcServiceTests` filter matched no tests but AgentService test project compiled.
- [x] **R6 — Clarify/fix cost cap accounting at ledger boundary** *(M)* — done 2026-06-22: `DbClaudeCostTracker` and `InMemoryClaudeCostTracker` now behave as observed-spend ledgers (append actual positive spend, do not clamp/drop over-cap entries); cap enforcement remains in `OrchestratorCostGuard` preflight/reservation path. Updated tests to expect over-cap MTD/percent >100 and non-positive-entry ignore behavior. Targeted tests passed: infrastructure cost tracker filter (5/5), `InMemoryClaudeCostTrackerTests` (6/6).
