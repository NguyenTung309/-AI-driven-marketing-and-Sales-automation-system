...
BEGIN
    -- Columns come from migrations 0033/0046; on an old local DB the repair block adds them
    -- right before this patch runs. Skip (without marking applied) if they are still missing.
    IF COL_LENGTH(N'dbo.messages', N'sender_display_name') IS NULL
        OR COL_LENGTH(N'dbo.messages', N'sender_avatar_url') IS NULL
        OR COL_LENGTH(N'dbo.contacts', N'avatar_url') IS NULL
    BEGIN
        PRINT 'messages.sender_* / contacts.avatar_url missing - patch skipped, retries next run.';
        RETURN;
    END
    ...
    -- (customer) message of the conversation. Dynamic SQL: referencing a missing column is a
    -- compile-time error even inside an IF, so this must stay out of the static batch.
    ...
    -- on the next activity. Dynamic SQL for the same compile-time reason as Fix A.
    EXEC sp_executesql N'
    UPDATE c
