...
CREATE TABLE pancake_configs (
    id                       UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWID(),
    tenant_id                UNIQUEIDENTIFIER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    channel                  NVARCHAR(32) NOT NULL DEFAULT N'pancake',
    page_id                  NVARCHAR(128) NOT NULL,
    base_url                 NVARCHAR(256) NOT NULL DEFAULT N'https://pancake.vn/api/v1',
    access_token_encrypted   NVARCHAR(MAX) NOT NULL,
    webhook_secret_encrypted NVARCHAR(MAX) NOT NULL,
    signature_header         NVARCHAR(64) NOT NULL DEFAULT N'x-pancake-signature',
    signature_algo           NVARCHAR(32) NOT NULL DEFAULT N'hmac-sha256',
    signature_encoding       NVARCHAR(16) NOT NULL DEFAULT N'hex',
    send_path_template       NVARCHAR(512) NOT NULL DEFAULT N'/pages/{page_id}/conversations/{thread_id}/messages',
    auth_mode                NVARCHAR(16) NOT NULL DEFAULT N'query',
    is_active                BIT NOT NULL DEFAULT 1,
    created_at               DATETIMEOFFSET NOT NULL DEFAULT SYSDATETIMEOFFSET(),
    updated_at               DATETIMEOFFSET NOT NULL DEFAULT SYSDATETIMEOFFSET(),
