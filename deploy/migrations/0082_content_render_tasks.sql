-- Persist durable content render tasks with tenant-safe item ownership and lease invariants.
-- One SqlCommand, no GO. Safe to re-run and able to complete a partially provisioned table.
...
BEGIN
    CREATE TABLE dbo.content_render_tasks (
        ...
        status NVARCHAR(24) NOT NULL
            CONSTRAINT DF_content_render_tasks_status DEFAULT N'pending',
        lease_token UNIQUEIDENTIFIER NULL,
        claimed_lease_token UNIQUEIDENTIFIER NULL,
        lease_expires_at DATETIMEOFFSET NULL,
        attempt_count INT NOT NULL
            CONSTRAINT DF_content_render_tasks_attempt_count DEFAULT 0,
...
IF COL_LENGTH(N'dbo.content_render_tasks', N'row_version') IS NOT NULL
   AND NOT EXISTS (
       ...
       WHERE c.object_id = OBJECT_ID(N'dbo.content_render_tasks')
         ...
         AND c.is_nullable = 0)
BEGIN
    DECLARE @rowVersionDefault SYSNAME;
    DECLARE @rowVersionDropSql NVARCHAR(MAX);
    SELECT @rowVersionDefault = dc.name
    FROM sys.default_constraints dc
    INNER JOIN sys.columns c
        ON c.object_id = dc.parent_object_id
       AND c.column_id = dc.parent_column_id
    ...
    IF @rowVersionDefault IS NOT NULL
    BEGIN
        SET @rowVersionDropSql = N'ALTER TABLE dbo.content_render_tasks DROP CONSTRAINT ' + QUOTENAME(@rowVersionDefault) + N';';
        EXEC (@rowVersionDropSql);
    END;
...
CREATE TABLE #content_render_task_expected_checks (
    ...
    CHECK (source_revision > 0
        AND source_revision < 2147483647
    ...
    CHECK (LEN(template_id) BETWEEN 1 AND 64
        AND LEN(template_hash) = 64
        AND template_hash COLLATE Latin1_General_100_BIN2 NOT LIKE N'%[^0-9a-f]%'
        AND LEN(slots_hash) = 64
        AND slots_hash COLLATE Latin1_General_100_BIN2 NOT LIKE N'%[^0-9a-f]%'
        AND ISJSON(canonical_slots_json, ARRAY) = 1
        AND slots_hash = LOWER(CONVERT(VARCHAR(64), HASHBYTES('SHA2_256',
            CONVERT(VARCHAR(MAX), canonical_slots_json COLLATE Latin1_General_100_BIN2_UTF8)), 2))
        AND DATALENGTH(CONVERT(VARCHAR(MAX),
            canonical_slots_json COLLATE Latin1_General_100_BIN2_UTF8)) <= 131072),
    CHECK (
        (status = N'pending'
        ...
        OR (status = N'leased'
            ...
            AND (claimed_lease_token IS NULL OR claimed_lease_token = lease_token)
        ...
        OR (status = N'completed'
            ...
            AND completed_revision = source_revision + 1)
        OR (status IN (N'failed', N'canceled_stale')
            AND lease_token IS NULL
            AND claimed_lease_token IS NULL
            AND lease_expires_at IS NULL
            AND completed_at IS NOT NULL
...
SELECT @expectedRevisionCheck = definition
FROM tempdb.sys.check_constraints
WHERE parent_object_id = OBJECT_ID(N'tempdb..#content_render_task_expected_checks')
  AND definition LIKE N'%source_revision%'
  AND definition NOT LIKE N'%completed_revision%';
SELECT @expectedStatusCheck = definition
FROM tempdb.sys.check_constraints
WHERE parent_object_id = OBJECT_ID(N'tempdb..#content_render_task_expected_checks')
  AND definition LIKE N'%canceled_stale%'
  AND definition NOT LIKE N'%lease_token%';
SELECT @expectedPresetCheck = definition
FROM tempdb.sys.check_constraints
WHERE parent_object_id = OBJECT_ID(N'tempdb..#content_render_task_expected_checks')
  AND definition LIKE N'%1200x630%';
SELECT @expectedPayloadCheck = definition
FROM tempdb.sys.check_constraints
WHERE parent_object_id = OBJECT_ID(N'tempdb..#content_render_task_expected_checks')
  AND definition LIKE N'%SHA2_256%';
SELECT @expectedStateCheck = definition
FROM tempdb.sys.check_constraints
WHERE parent_object_id = OBJECT_ID(N'tempdb..#content_render_task_expected_checks')
  AND definition LIKE N'%lease_token%';

IF @expectedRevisionCheck IS NULL OR @expectedStatusCheck IS NULL OR @expectedPresetCheck IS NULL
   OR @expectedPayloadCheck IS NULL OR @expectedStateCheck IS NULL
...
IF EXISTS (
    ...
    WHERE name = N'CK_content_render_tasks_revision'
      ...
      AND (is_disabled = 1 OR is_not_trusted = 1 OR definition <> @expectedRevisionCheck))
...
IF EXISTS (
    ...
    WHERE name = N'CK_content_render_tasks_status'
      AND parent_object_id = OBJECT_ID(N'dbo.content_render_tasks')
      AND (is_disabled = 1 OR is_not_trusted = 1 OR definition <> @expectedStatusCheck))
...
IF EXISTS (
    ...
    WHERE name = N'CK_content_render_tasks_preset'
      AND parent_object_id = OBJECT_ID(N'dbo.content_render_tasks')
      AND (is_disabled = 1 OR is_not_trusted = 1 OR definition <> @expectedPresetCheck))
...
IF EXISTS (
    ...
    WHERE name = N'CK_content_render_tasks_payload'
      AND parent_object_id = OBJECT_ID(N'dbo.content_render_tasks')
      AND (is_disabled = 1 OR is_not_trusted = 1 OR definition <> @expectedPayloadCheck))
...
IF EXISTS (
    ...
    WHERE name = N'CK_content_render_tasks_state'
      AND parent_object_id = OBJECT_ID(N'dbo.content_render_tasks')
      AND (is_disabled = 1 OR is_not_trusted = 1 OR definition <> @expectedStateCheck))
      ...
      AND (fk.referenced_object_id <> OBJECT_ID(N'dbo.content_items')
           ...
           OR NOT EXISTS (
               ...
               WHERE fkc.constraint_object_id = fk.object_id
                 ...
                 AND COL_NAME(fkc.parent_object_id, fkc.parent_column_id) = N'tenant_id'
                 AND COL_NAME(fkc.referenced_object_id, fkc.referenced_column_id) = N'tenant_id')
           OR NOT EXISTS (
               SELECT 1 FROM sys.foreign_key_columns fkc
               WHERE fkc.constraint_object_id = fk.object_id
                 AND fkc.constraint_column_id = 2
                 AND COL_NAME(fkc.parent_object_id, fkc.parent_column_id) = N'content_item_id'
...
IF EXISTS (
    ...
    WHERE i.name = N'UX_content_items_tenant_id_id'
      AND i.object_id = OBJECT_ID(N'dbo.content_items')
...
IF NOT EXISTS (
    ...
    WHERE name = N'CK_content_render_tasks_revision'
      AND parent_object_id = OBJECT_ID(N'dbo.content_render_tasks'))
BEGIN
    EXEC(N'ALTER TABLE dbo.content_render_tasks WITH CHECK
        ADD CONSTRAINT CK_content_render_tasks_revision
        CHECK (source_revision > 0
            AND source_revision < 2147483647
            AND template_version > 0
            AND attempt_count >= 0);');
...
BEGIN
    EXEC(N'ALTER TABLE dbo.content_render_tasks WITH CHECK
        ADD CONSTRAINT CK_content_render_tasks_status
        CHECK (status IN (N''pending'', N''leased'', N''completed'', N''failed'', N''canceled_stale''));');
END;
...
BEGIN
    EXEC(N'ALTER TABLE dbo.content_render_tasks WITH CHECK
        ADD CONSTRAINT CK_content_render_tasks_preset
        CHECK (preset IN (N''1200x630'', N''1080x1080''));');
END;
...
BEGIN
    EXEC(N'ALTER TABLE dbo.content_render_tasks WITH CHECK
        ADD CONSTRAINT CK_content_render_tasks_payload
        CHECK (LEN(template_id) BETWEEN 1 AND 64
            AND LEN(template_hash) = 64
            AND template_hash COLLATE Latin1_General_100_BIN2 NOT LIKE N''%[^0-9a-f]%''
            AND LEN(slots_hash) = 64
            AND slots_hash COLLATE Latin1_General_100_BIN2 NOT LIKE N''%[^0-9a-f]%''
            AND ISJSON(canonical_slots_json, ARRAY) = 1
            AND slots_hash = LOWER(CONVERT(VARCHAR(64), HASHBYTES(
                ''SHA2_256'',
                CONVERT(VARCHAR(MAX), canonical_slots_json COLLATE Latin1_General_100_BIN2_UTF8)), 2))
            AND DATALENGTH(CONVERT(VARCHAR(MAX),
                canonical_slots_json COLLATE Latin1_General_100_BIN2_UTF8)) <= 131072);');
...
BEGIN
    EXEC(N'ALTER TABLE dbo.content_render_tasks WITH CHECK
        ADD CONSTRAINT CK_content_render_tasks_state
        CHECK (
            (status = N''pending''
                AND lease_token IS NULL
                AND claimed_lease_token IS NULL
...
IF EXISTS (
    SELECT 1 FROM sys.indexes i
    WHERE i.name = N'UX_content_render_tasks_item_revision'
      AND i.object_id = OBJECT_ID(N'dbo.content_render_tasks')
      AND (i.is_unique = 0 OR i.type_desc <> N'NONCLUSTERED' OR i.is_disabled = 1 OR i.has_filter = 1
...
IF EXISTS (
    SELECT 1 FROM sys.indexes i
    WHERE i.name = N'IX_content_render_tasks_due'
      AND i.object_id = OBJECT_ID(N'dbo.content_render_tasks')
...
IF EXISTS (
    SELECT 1 FROM sys.indexes i
    WHERE i.name = N'IX_content_render_tasks_expired_lease'
      AND i.object_id = OBJECT_ID(N'dbo.content_render_tasks')
      AND (i.is_unique = 1 OR i.type_desc <> N'NONCLUSTERED' OR i.is_disabled = 1 OR i.has_filter = 1
           OR (SELECT COUNT(*) FROM sys.index_columns ic WHERE ic.object_id = i.object_id AND ic.index_id = i.index_id AND ic.key_ordinal > 0) <> 3
           OR EXISTS (SELECT 1 FROM sys.index_columns ic WHERE ic.object_id = i.object_id AND ic.index_id = i.index_id AND ic.is_included_column = 1)
...
BEGIN
    EXEC(N'ALTER TABLE dbo.content_render_tasks WITH CHECK
        ADD CONSTRAINT FK_content_render_tasks_content_items_tenant_item
