...
BEGIN
    DECLARE @ChannelDefaultConstraintName nvarchar(200);
    SELECT @ChannelDefaultConstraintName = dc.name
    FROM sys.default_constraints dc
    INNER JOIN sys.columns c ON c.default_object_id = dc.object_id
    WHERE dc.parent_object_id = OBJECT_ID('pancake_configs')
      AND c.name = 'channel';

    IF @ChannelDefaultConstraintName IS NOT NULL
        EXEC('ALTER TABLE pancake_configs DROP CONSTRAINT ' + @ChannelDefaultConstraintName);
