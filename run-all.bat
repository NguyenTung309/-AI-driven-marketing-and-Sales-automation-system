...
type "%ROOT%deploy\backfill_content_agent_review.sql" | docker exec -i clawbot-sqlserver %SQLCMD% -S localhost -U sa -P "%MSSQL_SA_PASSWORD%" -C -d clawbot -b
if errorlevel 1 (
    echo [ERROR] Data patch failed: deploy\backfill_content_agent_review.sql
    exit /b 1
)
exit /b 0
