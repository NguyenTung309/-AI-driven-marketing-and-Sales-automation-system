  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center overflow-hidden bg-surface relative">
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-0 pointer-events-none opacity-[0.03] w-4/5 max-w-[800px] flex flex-col items-center select-none">
        <span aria-hidden="true" className="material-symbols-outlined text-[300px] [font-variation-settings:'wght'_700]">school</span>
        <h1 className="text-9xl font-bold uppercase tracking-widest mt-8 text-center">Học Bá AI</h1>
      </div>
      ...
      <footer className="absolute bottom-0 left-0 right-0 w-full flex flex-col md:flex-row justify-between items-center gap-4 px-8 py-4 z-10 text-on-surface-variant">
        <span className="text-label-lg font-bold text-on-surface">Học Bá AI</span>
        <nav className="flex gap-6">
          <a href="#" className="text-label-sm hover:text-primary hover:underline transition-colors">Chính sách bảo mật</a>
          <a href="#" className="text-label-sm hover:text-primary hover:underline transition-colors">Điều khoản sử dụng</a>
          <a href="#" className="text-label-sm hover:text-primary hover:underline transition-colors">Hỗ trợ</a>
        </nav>
        <span className="text-label-sm">© 2024 Học Bá AI. Bảo lưu mọi quyền.</span>
      </footer>
    </div>
  );
}
