...
public static class ToolRegistryFactory
{
    ...
    private static readonly Dictionary<string, (string Description, string Permission, ToolRiskLevel Risk)> Metadata =
        new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            // Hai operation content_* là đường duy nhất ra số liệu marketing: snapshot/anomaly/forecast
            // đều đọc kpi_daily, tức chỉ có chỉ số sale, nên goal về nội dung mà rơi vào snapshot sẽ trả
            // về bảng lead/hội thoại — đúng cú pháp nhưng sai nghiệp vụ.
            ["report-agent"] = ("Report on business KPIs or on marketing content. operation=snapshot|anomaly|forecast read the SALE KPI table (leads/dms/replies/conversions/avg_response_time_sec only, no infrastructure KPIs). operation=snapshot supports single date or date range/week/month (use date='today'|'yesterday'|'this_week'|'last_week'|'this_month'|'last_month' or YYYY-MM-DD, or from_date/to_date, or lookback_days). metric is required for anomaly/forecast. operation=content_snapshot reports PUBLISHED POSTS + engagement (likes/comments/reactions) per platform, and operation=content_funnel reports how many drafts sit at each review/approval stage — use these two for any marketing/content goal. platform defaults to all; date defaults to today. The result carries reportUrl — always quote that link in your final answer so the user can open the full table/chart and download Excel/PDF.", "analytics:read", ToolRiskLevel.Low),
    ...
    {
        ["report-agent"] = """
            {"type":"object","properties":{"operation":{"type":"string","enum":["snapshot","anomaly","forecast","content_snapshot","content_funnel"],"default":"snapshot","description":"snapshot = KPI sale/leads/dms/chuyển đổi (theo ngày hoặc khoảng tuần/tháng); anomaly = phát hiện bất thường; forecast = dự báo; content_snapshot = bài đã đăng + tương tác theo nền tảng (MARKETING); content_funnel = số bài đang tắc ở từng khâu duyệt (MARKETING). Mọi yêu cầu về nội dung/bài đăng/marketing phải dùng content_snapshot hoặc content_funnel, KHÔNG dùng snapshot"},"metric":{"type":"string","enum":["leads","dms","replies","conversions","avg_response_time_sec"],"description":"Bắt buộc cho anomaly/forecast. Chỉ nhận đúng các giá trị này — không có metric hạ tầng (uptime, error_rate, throughput) và không áp dụng cho content_*"},"platform":{"type":"string","description":"facebook|instagram|zalo|website|... Bỏ trống = 'all' (tổng hợp mọi nền tảng)"},"date":{"type":"string","description":"snapshot: ngày cụ thể YYYY-MM-DD hoặc từ khóa 'today'/'yesterday'/'this_week'/'last_week'/'this_month'/'last_month'. content_*: ngày cuối của khoảng. Bỏ trống = hôm nay"},"from_date":{"type":"string","description":"snapshot: ngày bắt đầu YYYY-MM-DD khi truy vấn khoảng ngày"},"to_date":{"type":"string","description":"snapshot: ngày kết thúc YYYY-MM-DD"},"lookback_days":{"type":"integer","description":"snapshot: số ngày lấy KPI (vd 7 cho tuần, 30 cho tháng). anomaly: số ngày lấy mẫu (mặc định 30). content_snapshot: độ dài khoảng (mặc định 7). content_funnel: bỏ trống"},"z_threshold":{"type":"number","description":"anomaly: ngưỡng z-score, mặc định 2.0"},"horizon_days":{"type":"integer","description":"forecast: số ngày dự báo, mặc định 7"}},"additionalProperties":false}
            """,
        ["content-agent"] = """
