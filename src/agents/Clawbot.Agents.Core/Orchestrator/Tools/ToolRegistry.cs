...
public static class ToolRegistryFactory
{
    ...
    private static readonly Dictionary<string, (string Description, string Permission, ToolRiskLevel Risk)> Metadata =
        new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["report-agent"] = ("Report on business KPIs or on marketing content. operation=snapshot|anomaly|forecast read the SALE KPI table (leads/dms/replies/conversions/avg_response_time_sec only, no infrastructure KPIs); metric is required for anomaly/forecast. operation=content_snapshot reports PUBLISHED POSTS + engagement (likes/comments/reactions) per platform, and operation=content_funnel reports how many drafts sit at each review/approval stage — use these two for any marketing/content goal, never snapshot. platform defaults to all; date defaults to today; lookback_days sets the window for the content operations. The result carries reportUrl — always quote that link in your final answer so the user can open the full table/chart and download Excel/PDF.", "analytics:read", ToolRiskLevel.Low),
    ...
    {
        ["report-agent"] = """
            {"type":"object","properties":{"operation":{"type":"string","enum":["snapshot","anomaly","forecast","content_snapshot","content_funnel"],"default":"snapshot","description":"snapshot = KPI sale theo ngày (lead/tin nhắn/chuyển đổi); anomaly = phát hiện bất thường; forecast = dự báo; content_snapshot = bài đã đăng + tương tác theo nền tảng (MARKETING); content_funnel = số bài đang tắc ở từng khâu duyệt (MARKETING). Mọi yêu cầu về nội dung/bài đăng/marketing phải dùng content_snapshot hoặc content_funnel, KHÔNG dùng snapshot"},"metric":{"type":"string","enum":["leads","dms","replies","conversions","avg_response_time_sec"],"description":"Bắt buộc cho anomaly/forecast. Chỉ nhận đúng các giá trị này — không có metric hạ tầng (uptime, error_rate, throughput) và không áp dụng cho content_*"},"platform":{"type":"string","description":"facebook|instagram|zalo|website|... Bỏ trống = 'all' (tổng hợp mọi nền tảng)"},"date":{"type":"string","pattern":"^\\d{4}-\\d{2}-\\d{2}$","description":"snapshot: đúng ngày đó. content_snapshot/content_funnel: ngày cuối của khoảng. Bỏ trống = hôm nay"},"lookback_days":{"type":"integer","description":"anomaly: số ngày lấy mẫu, mặc định 30. content_snapshot: độ dài khoảng, mặc định 7. content_funnel: bỏ trống = toàn bộ bài còn tồn đọng (nên bỏ trống), chỉ đặt khi người dùng hỏi riêng về bài tạo gần đây"},"z_threshold":{"type":"number","description":"anomaly: ngưỡng z-score, mặc định 2.0"},"horizon_days":{"type":"integer","description":"forecast: số ngày dự báo, mặc định 7"}},"additionalProperties":false}
            """,
        ["content-agent"] = """
