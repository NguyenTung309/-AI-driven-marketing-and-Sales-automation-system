...
public interface ICommentChannelAdapter
{
    ...
    Task<string?> SendCommentReplyAsync(string externalThreadId, string commentMessageId, string text, CancellationToken ct = default);

    /// <summary>Private reply từ comment — FB giới hạn 1 lần/comment trong window 7 ngày.</summary>
    Task<string?> SendPrivateReplyAsync(string externalThreadId, string postId, string commentMessageId, string fromId, string text, CancellationToken ct = default);
}
